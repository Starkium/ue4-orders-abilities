...
float URTSUtilities::GetActorCollisionSize(const AActor* Actor)
{
    if (!Actor)
    {
        return 0.0f;
...
bool URTSUtilities::AreInSameTeam(AActor* FirstActor, AActor* SecondActor)
{
	const URTSOwnerComponent* FirstOwner = FirstActor->FindComponentByClass<URTSOwnerComponent>();
	const URTSOwnerComponent* SecondOwner = SecondActor->FindComponentByClass<URTSOwnerComponent>();
	if (!FirstOwner || !SecondOwner)
	{
		return false;
	}

	if (!FirstOwner->GetPlayerOwner() && !SecondOwner->GetPlayerOwner())
	{
		return true;
	}
	...
	const ARTSTeamInfo* FirstTeam = FirstOwner->GetPlayerOwner()->Team;
	const ARTSTeamInfo* SecondTeam = SecondOwner->GetPlayerOwner()->Team;
